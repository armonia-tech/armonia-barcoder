...
class Barcoder
{
    ...
    public function getBarcodeData($w = 2, $h = 100, $color = array(0, 0, 0), $image_format = 'PNG')
    {
        if (!empty($this->font_array['has_print_text'])) {
            ...
            $image_height = abs($type_space[5] - $type_space[1]) + $font_top_spacing;
        } else {
            $image_height = 0;
        }
    ...
    public function setBarcode($code, $type)
    {
        switch (strtoupper($type)) {
            ...
            case 'IMBPRE': // IMB - Intelligent Mail Barcode - Onecode - USPS-B-3200- pre-processed
                $arrcode = $this->barcodeImbPre($code);
                break;
    ...
    protected function barcodeCode39($code, $extended = false, $checksum = false)
    {
        ...
        if ($extended) {
            // extended mode
            $code = $this->encodeCode39Exts($code);
        }
        if ($code === false) {
            return false;
        }
